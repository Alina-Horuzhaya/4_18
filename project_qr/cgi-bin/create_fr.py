...
def generate_qr_and_save_img(values_from_form: dict = None) -> str:
    ...
    data = "BEGIN:VCARD\n" + "VERSION:3.0\n" + "FN:" + str(values_from_form["fio"]) + '\nTEL;TYPE=cell:' \
           + str(values_from_form["cell"]) + '\nEMAIL;TYPE=Email:' + str(values_from_form["email"]) \
           + '\nX-SOCIALPROFILE;X-USER=@' + str(values_from_form["social"]) + ";TYPE=Telegram:https://t.me/" \
           + str(values_from_form["social"]) + "\nNOTE:Спецификация: " + str(values_from_form["note_1"]) \
           + " | Опыт работы:" + str(values_from_form["note_2"]) + "\nEND:VCARD"
    filename = "qr_fr_" + str(values_from_form["fio"]) + ".png"
    # создаем код
...
## Обработчик формы создания стандартной визитки. получение данных из формы create_fr.html
#  @return: filename (str) - название файла
def process_form_and_generate_qr() -> str:
    ...
    values_dict = {"fio": form.getfirst("FIO"), "cell": form.getfirst("CELL"), "email": form.getfirst("EMAIL"),
                   "social": form.getfirst("SOCIAL", "-"), "note_1": form.getfirst("NOTE_1", "-"),
                   "note_2": form.getfirst("NOTE_2", "-")}
...
print("""<!DOCTYPE HTML>
        ...
        <head>
            <meta charset="utf-8">
            <title>Создание визитки для фриланса</title>
            <style>
...
print("</a>")
print("<br><button><a href=\create_fr.html>")
