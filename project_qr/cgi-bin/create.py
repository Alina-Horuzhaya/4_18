...
# '\nX-SOCIALPROFILE;X-USER=@' + str(values_from_form["social"]) + \
# ";TYPE=Telegram:https://t.me/" + str(values_from_form["social"]) + \
...
def generate_qr_and_save_img(values_from_form: dict = None) -> str:
    ...
    data = "BEGIN:VCARD\n" + "VERSION:3.0\n" + "FN:" + str(values_from_form["fio"]) + \
           ...
           "\nitem2.URL;TYPE=Ссылка на сервис:" + str(values_from_form["port"]) + \
           "\nNOTE:" + str(values_from_form["note"]) + "\nEND:VCARD"
    filename = "qr_" + str(values_from_form["fio"]) + ".png"
    # создаем код
...
## Обработчик формы создания стандартной визитки. получение данных из формы create.html
#  @return: filename (str) - название файла
def process_form_and_generate_qr() -> str:
    form = cgi.FieldStorage()
    values_dict = {"fio": form.getfirst("FIO"), "cell": form.getfirst("CELL"),
                   "port": form.getfirst("PORT", "-"), "email": form.getfirst("EMAIL", "-"),
...
print("""<!DOCTYPE HTML>
        ...
        <head>
            <meta charset="utf-8">
            <title>Создание стандартной визитки</title>
            <style>
...
print("</a>")
print("<br> <button><a href=\create.html>")
